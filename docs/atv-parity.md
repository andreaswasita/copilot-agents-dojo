# ATV Parity — Gap Analysis & Roadmap
...
> [`All-The-Vibes/ATV-StarterKit`](https://github.com/All-The-Vibes/ATV-StarterKit)
>
...
ATV ("All The Vibes") Starter Kit and this Dojo solve overlapping problems from
opposite ends:

- **ATV** optimizes for **distribution and developer experience** — a
  one-command, zero-dependency installer that wires four upstream pillars
  (autoresearch, compound-engineering, gstack, agent-browser) into a complete
  agentic coding environment, with a marketplace, a TUI, lifecycle management,
  and a telemetry-driven learning loop.
...
spec gate back to ATV as upstream PR #55). This analysis captures **where ATV is
ahead**, so we can close the distribution/UX/lifecycle gap without giving up the
governance lead.
...
| Dimension | ATV-StarterKit | Copilot Agents Dojo (this repo) |
|---|---|---|
| Primary CLI | Compiled **Go binary** (GoReleaser, cross-platform) | **Python** CLI (`pip install -e`) |
| Install entry | `npx atv-starterkit init` (downloads platform binary, no toolchain) | `install.sh`/`install.ps1` + `npx copilot-dojo init` (newer/thinner) |
| Installer UX | **Guided TUI** — wizard, presets, multi-stack selection | Non-interactive scripts |
| Lifecycle | Install manifest + **drift detection** + `/atv-doctor` + `/atv-update` + preserve-modified uninstall | **None** (no manifest, drift, or uninstall) |
| Marketplace | Tiered plugins (everything / agents / packs / per-skill) across Copilot CLI + VS Code + Claude, drift-validated | Minimal; `plugin.json`/`marketplace.json` not on `main` |
| Learning loop | Observer hooks → `observations.jsonl` → instincts (confidence) → `/evolve` into skills | Curator lifecycle + manual lessons (no tool-use capture, no confidence promotion) |
| Security skill | `/atv-security` — 33 config rules + OWASP Top-10 + STRIDE, deterministic+LLM, persisted reports, `fix` mode | `derive-security-from-risk` (requirements-side only — no scanner) |
| Browser/QA | Bundled agent-browser + `/gstack-qa` / `/gstack-browse` / `/gstack-benchmark` | None |
| Pipeline orchestration | `/lfg` and `/slfg` (parallel swarm) chain the full sprint | Discrete workflow skills, no single orchestrator |
| Surface | 45+ skills / 51 agents as slash commands + generated `.prompt.md` shims | 26 skills / 8 personas, no slash/prompt-shim layer |
| Onboarding | External **guided training quest** (gamified) | README + docs |
| **Spec/governance gate** | Adopted (via our upstream PR #55) | **Formal spec + `verify.sh` invariant gate** ✅ lead |
...
## 3. Gaps (what ATV has that we lack)

Each gap is tagged with its target theme and a rough effort (S/M/L).

| # | Gap | Theme | Effort | Why it matters |
|---|---|---|---|---|
| G1 | **Zero-dependency binary distribution** — ship a single self-contained CLI installable with no Python/Node toolchain | T1 | L | Python `pip install -e` is the #1 adoption friction; ATV's "no toolchain" `npx` path is its biggest DX advantage |
...
| G5 | **Telemetry-driven learning loop** — observer hooks → observations → confidence-scored instincts → graduate into skills | T3 | L | Our self-improvement is manual; ATV's loop makes the repo measurably smarter per session |
...
| G7 | **Browser automation / live QA skills** — drive a real browser for QA, screenshots, benchmarks | T2 | M | Whole test surface ATV covers and we don't |
| G8 | **One-command pipeline orchestration** — a `/lfg`-style command chaining plan→…→review→test→learn, plus a parallel variant | T2 | M | We have the steps but no single "run the sprint" entry point |
| G9 | **Slash-command + prompt-shim surface** — expose skills/personas as discoverable slash commands with generated shims | T2/T1 | M | Discoverability and activation; ATV's primary UX |
| G10 | **Autonomous metric hill-climbing** — `/autoresearch`-style experiment loop on a dedicated branch with a results log | T3 | L | Differentiated capability for measurable tasks (perf, bundle size, pass rate) |
...
NOW   G6 security audit
                                      NEXT G7 browser/QA
                                      LATER G5 learning loop ; G10 autoresearch
```
...
- **Bundling four upstream pillars** (gstack, agent-browser, compound-engineering,
  autoresearch) wholesale — that is ATV's product identity, not ours. We adopt
  the *capabilities* (QA, orchestration, learning) natively where they fit the
  governance model, rather than vendoring external runtimes.
- **Claude-plugin marketplace format** as the primary surface — we target
...
ATV is ahead on distribution, installer UX, lifecycle, marketplace, telemetry
learning, and ready-made security/QA/orchestration skills. We are ahead on
governance rigor, traceability, and the control plane. This roadmap closes the
former without conceding the latter.
